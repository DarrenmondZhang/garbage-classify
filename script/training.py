...
from utils.train_eval import train, evaluate, initital_model, class_id2name
## 工具类： 日志类工具、模型保存、优化器
...
def run(model, train_loader, val_loader):
    ...
    # 2. 加载checkpoint： 可以指定迭代的开始位置进行重新训练
    if args.resume:
